...
import type { QimenInput } from '@bazi/domain';
import { CitySelector } from '@/components/bazi/city-selector';
import type { City } from '@/data/cities';

interface QimenFormProps {
  onSubmit: (input: QimenInput) => void;
  isLoading: boolean;
}

export function QimenForm({ onSubmit, isLoading }: QimenFormProps) {
  const t = useTranslations('qimen.form');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const input: QimenInput = { year, month, day, hour, minute, question: question || undefined };
        ...
        {showAdvanced && (
          <div className="space-y-3 border-t border-gray-200 bg-gray-50 p-4">
            <div className="flex items-center gap-2">
              <input
                type="checkbox"
                id="qimenTrueSolarTime"
                checked={useTrueSolarTime}
                onChange={(e) => setUseTrueSolarTime(e.target.checked)}
                className="rounded border-gray-300"
              />
              <label htmlFor="qimenTrueSolarTime" className="text-sm text-gray-700">
                {t('useTrueSolarTime')}
              </label>
            </div>
            ...
            {useTrueSolarTime && (
              <>
                ...
                <p className="text-[11px] text-gray-400">{t('southernHemisphereNote')}</p>
              </>
            )}
          </div>
        )}
