...
import type { MarriageCompatibility, BaZiResult } from '@bazi/domain';
import { PillarChart } from './pillar-chart';

interface MarriageResultDisplayProps {
...
export function MarriageResultDisplay({
  compatibility,
  maleResult,
  femaleResult,
}: MarriageResultDisplayProps) {
      ...
      <section className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="rounded-xl border border-gray-200 bg-white p-6">
          <h3 className="mb-4 text-sm font-semibold text-gray-700">
            {t('labels.malePillars')}
          </h3>
          <PillarChart
            fourPillars={maleResult.chart.fourPillars}
            dayMaster={maleResult.chart.dayMaster}
          />
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-6">
          <h3 className="mb-4 text-sm font-semibold text-gray-700">
            {t('labels.femalePillars')}
          </h3>
          <PillarChart
            fourPillars={femaleResult.chart.fourPillars}
            dayMaster={femaleResult.chart.dayMaster}
          />
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        >
          <div className="grid grid-cols-2 gap-2">
            <div>
              <span className="text-gray-400">{t('labels.maleDayMaster')}：</span>
              {dayMasterRelation.maleStem}
            </div>
            <div>
              <span className="text-gray-400">{t('labels.femaleDayMaster')}：</span>
              {dayMasterRelation.femaleStem}
            </div>
            <div>
            ...
            <div>
              <span className="text-gray-400">{t('labels.maleToFemale')}：</span>
              {dayMasterRelation.shiShenMaleToFemale}
            </div>
            <div className="col-span-2">
              <span className="text-gray-400">{t('labels.femaleToMale')}：</span>
              {dayMasterRelation.shiShenFemaleToMale}
            </div>
          </div>
        </DimensionCard>
        ...
        >
          <div className="grid grid-cols-2 gap-2">
            <div>
              <span className="text-gray-400">{t('labels.maleYongShen')}：</span>
              {yongShenMatch.maleYongShen}
            </div>
            <div>
              <span className="text-gray-400">{t('labels.femaleYongShen')}：</span>
              {yongShenMatch.femaleYongShen}
            </div>
            <div>
              <span className="text-gray-400">{t('labels.maleXiShen')}：</span>
              {yongShenMatch.maleXiShen}
            </div>
            <div>
              <span className="text-gray-400">{t('labels.femaleXiShen')}：</span>
              {yongShenMatch.femaleXiShen}
            </div>
          </div>
        </DimensionCard>
        ...
        >
          <div className="grid grid-cols-2 gap-2">
            <div>
              <span className="text-gray-400">{t('labels.maleNaYin')}：</span>
              {nayinMatch.maleNaYin}
            </div>
            <div>
              <span className="text-gray-400">{t('labels.femaleNaYin')}：</span>
              {nayinMatch.femaleNaYin}
            </div>
            <div className="col-span-2">
